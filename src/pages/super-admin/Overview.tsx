    ...
    return (
        <div className="space-y-8 pb-12">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                        <Badge className="bg-[#7ede56] text-[#002f37] border-none font-black text-[10px] px-2 shadow-sm">LIVE SYSTEM STATUS</Badge>
                        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest flex items-center gap-1">
                            <Activity className="w-3 h-3 text-emerald-500 animate-pulse" /> Uptime: {stats?.systemUptime || 'Stable'}
                        </span>
                    </div>
                </div>
                <div className="flex gap-2">
                    <Button className="bg-[#002f37] dark:bg-[#7ede56] dark:text-[#002f37] hover:opacity-90 font-black uppercase text-[10px] tracking-widest h-11 px-8 shadow-premium rounded-2xl">
                ...
                {metrics.map((metric, index) => (
                    ...
                    >
                        ...
                        <CardHeader className="p-4 pb-0 relative z-10 flex flex-row items-center justify-between space-y-0">
                            <div className={`p-2 rounded-lg ${metric.id === 'active' ? 'bg-[#002f37]/10' : 'bg-white/10'} backdrop-blur-md`}>
                                <metric.icon className={`h-4 w-4 ${metric.iconColor}`} />
                            </div>
                            <ArrowUpRight className={`h-4 w-4 ${metric.id === 'active' ? 'text-[#002f37]/40' : 'text-white/40'}`} />
                        </CardHeader>

                        <CardContent className="p-4 pt-4 relative z-10 flex flex-col justify-end h-[calc(9rem-3rem)] md:h-[calc(11.5rem-3rem)]">
                            <p className={`text-[9px] font-black uppercase tracking-[0.2em] mb-1 truncate ${metric.id === 'active' ? 'text-[#002f37]/70' : 'text-white/80'}`}>
                                {metric.title}
                            </p>
                            <h3 className={`text-2xl md:text-3xl font-black ${metric.textColor}`}>
                                <CountUp end={metric.value} duration={1500} />
                            </h3>
                            <p className={`text-[9px] font-bold mt-2 line-clamp-1 uppercase tracking-tighter ${metric.id === 'active' ? 'text-[#002f37]/50' : 'text-white/60'}`}>
                                {metric.description}
                            </p>
                        </CardContent>
                    </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <Card className={`col-span-1 lg:col-span-2 border-none shadow-premium overflow-hidden ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
                    <CardHeader className="pb-4 border-b border-gray-100 dark:border-gray-800">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-2">
                                <Badge variant="secondary" className="font-black text-[9px] uppercase tracking-widest px-2 py-0.5">National Distribution</Badge>
                            </div>
                        </div>
                    </CardHeader>
                        ...
                        <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                            {stats?.regionalDistribution?.map((item: any, idx: number) => (
                                <div key={item.name} className="group flex flex-col gap-1.5">
                                    ...
                                    <div className="h-3 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden flex items-center relative">
                                        <div
                                            className="h-full bg-gradient-to-r from-[#002f37] via-[#065f46] to-[#7ede56] rounded-full transition-all duration-1000 group-hover:shadow-[0_0_15px_rgba(126,222,86,0.4)]"
                                            style={{ width: `${(item.value / 1000) * 100}%` }}
                                        >
                                            <div className="h-full w-full flex">
                                                <div className="h-full flex-1 border-r border-white/10"></div>
                                                <div className="h-full flex-1 border-r border-white/10 opacity-60"></div>
                                                <div className="h-full flex-1 opacity-30"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <Card className={`border-none shadow-premium h-full ${darkMode ? 'bg-gray-900 border border-gray-800' : 'bg-white'}`}>
                        <CardHeader className="pb-4 border-b border-gray-100 dark:border-gray-800">
                            <div className="flex items-center gap-2">
                                ...
                                <CardTitle className={`text-sm font-black uppercase tracking-widest ${darkMode ? 'text-white' : 'text-[#002f37]'}`}>Platform Snapshot</CardTitle>
                            </div>
                        </CardHeader>
                        <CardContent className="pt-8 space-y-6">
                            <div className="space-y-2">
                                ...
                                <div className="h-2 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                                    <div className="h-full bg-gradient-to-r from-blue-500 to-emerald-500 w-[92.4%] rounded-full shadow-[0_0_8px_rgba(16,185,129,0.3)]"></div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="h-2 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                                    <div className="h-full bg-gradient-to-r from-emerald-500 to-[#7ede56] w-[98.1%] rounded-full shadow-[0_0_8px_rgba(126,222,86,0.3)]"></div>
                                </div>
                            </div>

                            <div className={`p-4 rounded-2xl ${darkMode ? 'bg-gray-800/50' : 'bg-gray-50'} border border-gray-100 dark:border-gray-800`}>
                                <div className="flex items-center gap-3 mb-2">
                                    <div className="w-2 h-2 rounded-full bg-[#7ede56] shadow-[0_0_8px_#7ede56] animate-pulse"></div>
                                    <span className="text-[10px] font-black uppercase tracking-widest">Global Watchdog</span>
                                </div>
                                <p className="text-[10px] font-bold text-gray-500 leading-relaxed uppercase tracking-tighter">
                                    All 8 regional clusters are communicating at optimal latency. No critical sync gaps detected in the last 24 hours.
                                </p>
                            </div>

                            <Button className="w-full h-12 bg-[#002f37] hover:bg-[#001c21] text-white font-black uppercase text-[10px] tracking-[0.2em] rounded-xl shadow-xl transition-all hover:scale-[1.02]">
