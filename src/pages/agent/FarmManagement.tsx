...
import {
  ...
  GROWER_DIRECTORY_LEGEND_ICON,
  growerDirectoryActionBtn,
  growerDirectoryMobileActionBtn,
...
const FarmManagement: React.FC = () => {
  ...
  const [verificationQueueModalOpen, setVerificationQueueModalOpen] =
    useState(false);
  const [selectedTab, setSelectedTab] = useState("personal");
  const [activeTab, setActiveTab] = useState<
    "farmers" | "farms" | "visits" | "reports" | "matches"
  >("farmers");
  ...
  useEffect(() => {
    ...
    if (
      tabParam &&
      ["farmers", "farms", "visits", "reports"].includes(tabParam)
    ) {
  ...
  const { data: deletionRequestsData, refetch: refetchDeletionRequests } =
    useQuery({
  ...
  const visitLogs = Array.isArray(visitLogsData)
    ? visitLogsData
    : visitLogsData?.data || [];
  ...
  const matches = useMemo(() => {
    ...
    const regSearch = (effectiveRegion || "")
      .toLowerCase()
      .replace(" region", "")
      .trim();
    return matchesRaw.filter((m: any) => {
      ...
      return (
        !regSearch ||
        mReg === regSearch ||
        mReg.includes(regSearch) ||
        regSearch.includes(mReg)
      );
    });
    ...
    return farmers
      ...
      .filter((farmer: any) => {
        ...
        const matchesRegion =
          !regSearch ||
          fReg === regSearch ||
          fReg.includes(regSearch) ||
          regSearch.includes(fReg);

        const matchesDistrict =
          selectedDistrict === "all" || farmer.district === selectedDistrict;
        const matchesCommunity =
          selectedCommunity === "all" || farmer.community === selectedCommunity;
        const matchesFarmType =
          selectedFarmType === "all" || farmer.farmType === selectedFarmType;
        const matchesCategory = selectedCategory === "all" || true;
        const filterStatus = statusFilter || selectedStatus;

        const matchesStatus =
          filterStatus === "all" || farmer.displayStatus === filterStatus;
        return (
          matchesSearch &&
          matchesRegion &&
          matchesDistrict &&
  ...
  const filteredFarms = useMemo(() => {
    return farms.filter((farm: any) => {
      ...
      const matchesDistrict =
        ...
        farm.farmer?.district === selectedDistrict;
  ...
  const getStatusBadgeColor = (status: string) => {
    switch (status) {
      ...
      case "Matched":
        return darkMode
          ? "bg-[#7ede56]/20 text-[#7ede56] border-0"
          : "bg-[#7ede56]/10 text-[#7ede56]";
      case "In Progress":
        return darkMode
  ...
  const handleRequestFarmerDeletion = async (farmer: any) => {
    ...
    const result = await Swal.fire({
      ...
      inputPlaceholder:
        "e.g. Duplicate record, grower relocated out of region, grower requested removal...",
      inputAttributes: {
        minlength: "10",
        maxlength: "2000",
    ...
    try {
      await api.post(`/farmers/${farmerId}/deletion-request`, {
      ...
      Swal.fire({
        ...
        text:
          error.response?.data?.message || "Could not submit deletion request.",
        confirmButtonColor: "#065f46",
      });
    }
  };
  ...
  const getFarmerMongoId = (farmer: any) =>
    farmer?._id?.toString?.() ??
    (typeof farmer?._id === "string" ? farmer._id : "");
  ...
  const handleEditVisit = (visit: any) => {
    ...
    const isOther =
      visit.purpose &&
      !standardPurposes.includes(visit.purpose) &&
      visit.purpose !== "Other";

    setVisitForm({
      farmerId:
        getFarmerMongoId(visit.farmer) || visit.farmerId?.toString?.() || "",
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.text(
        `AUTHORIZED BY: ${(agent?.name || "AgriLync Agent").toUpperCase()}`,
        pageWidth / 2,
        68,
        { align: "center" },
      );
      ...
      autoTable(doc, {
        ...
        didDrawPage: (data) => {
          ...
          doc.setFont("helvetica", "normal");
          doc.text(
            `Report for: ${agent?.name || "Agent"}  |  Exported: ${new Date().toLocaleString()}`,
            28,
            pageHeight - 13,
          );
      ...
      doc.save(
        `AgriLync_VisitLogs_${new Date().toISOString().split("T")[0]}.pdf`,
      );
      await Swal.fire({
        icon: "success",
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      XLSX.writeFile(
        wb,
        `AgriLync_VisitLogs_${new Date().toISOString().split("T")[0]}.xlsx`,
      );
      await Swal.fire({
        icon: "success",
        title: "Export Successful!",
      ...
      await showAutoSuccessAlert(
        ...
        `<p style="font-size: 16px; color: #065f46; margin: 8px 0;">${
          visitForm.isEditing
            ...
            : "Field visit logged successfully!"
        }</p>`,
        3000,
      );
      queryClient.invalidateQueries({ queryKey: ["fieldVisits"] });
  ...
  return (
    <AgentLayout activeSection="farm-management" title="Manage Farm">
      <div className="space-y-8">
            ...
            !isLoaded ? (
              ...
              >
                <div className="absolute -right-4 -bottom-4 opacity-10 group-hover:scale-110 transition-transform">
                  <item.icon
                    className={`h-20 w-20 sm:h-24 sm:w-24 ${item.title === "Total Farmers" ? "text-white" : item.color.replace("bg-", "text-")} -rotate-12`}
                  />
                </div>

                <div className="flex items-center justify-between">
                  <div
                    className={`p-1.5 sm:p-2 ${item.title === "Total Farmers" ? "bg-white/10" : item.color.replace("bg-", "bg-").concat("/10")} rounded-lg`}
                  >
                    <item.icon
                      className={`h-4 w-4 sm:h-5 sm:w-5 ${item.title === "Total Farmers" ? "text-white" : item.color.replace("bg-", "text-")}`}
                    />
                  </div>
                  <span
                    className={`text-[8px] sm:text-[10px] font-black ${item.title === "Total Farmers" ? "text-white/40" : "text-gray-400"} uppercase tracking-widest`}
                  >
                    STATUS
                  </span>
                </div>
                ...
                <div>
                  <p
                    className={`text-[8px] sm:text-[10px] font-black ${item.title === "Total Farmers" ? "text-white/60" : "text-gray-500"} uppercase tracking-widest mb-0.5 sm:mb-1`}
                  >
                    {item.title}
                  </p>
                  <div className="flex items-baseline gap-1 sm:gap-2">
                    <h3
                      className={`text-xl sm:text-4xl font-black ${item.title === "Total Farmers" ? "text-white" : "text-gray-900"} leading-none`}
                    >
                      <CountUp end={Number(item.value)} duration={1000} />
                    </h3>
                    <span
                      className={`text-[8px] sm:text-[10px] font-bold ${item.title === "Total Farmers" ? "text-white/80" : "text-gray-500"}`}
                    >
                      {item.subtext}
                    </span>
                  </div>
        ...
        <Tabs
          ...
          onValueChange={(v) =>
            setActiveTab(
              v as "farmers" | "farms" | "visits" | "reports" | "matches",
            )
          }
          className="w-full"
        >
          <TabsList
            className={`flex w-full overflow-x-auto whitespace-nowrap scrollbar-hide bg-transparent p-0 h-auto gap-2 sm:gap-4 mb-4 sm:mb-8 border-b ${darkMode ? "border-white/10" : "border-gray-200"}`}
          >
            <TabsTrigger
              value="farmers"
              className={`flex-shrink-0 px-4 sm:px-8 py-3 sm:py-4 text-xs sm:text-sm font-bold border-b-2 rounded-none transition-all ${
                activeTab === "farmers"
          ...
          <TabsContent value="farmers" className="space-y-4">
            ...
            <Card
              className={`${sectionCardClass} border-0 shadow-lg overflow-hidden`}
            >
              <div className="p-4 sm:p-6">
                <div className="flex flex-col xl:flex-row gap-4 xl:items-center justify-between">
                  <div className="relative flex-1 group">
                    <Search
                      className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 transition-colors ${darkMode ? "text-gray-500 group-focus-within:text-[#7ede56]" : "text-gray-400 group-focus-within:text-[#7ede56]"}`}
                    />
                    <Input
                      placeholder="Search by name, phone or Lync ID..."
                      className={`pl-10 h-11 ${inputBaseClasses} ${darkMode ? "bg-white/5 border-white/10 focus:ring-[#7ede56]/50" : "bg-gray-50"}`}
                      value={searchQuery}
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    >
                      <SelectTrigger
                        className={`w-full sm:w-40 h-11 ${inputBaseClasses} ${darkMode ? "bg-white/5 border-white/10" : "bg-gray-50"}`}
                      >
                        <SelectValue placeholder="District" />
                      </SelectTrigger>
                      <SelectContent className="z-[2000] rounded-xl border-none shadow-2xl">
                        <SelectItem value="all">All Districts</SelectItem>
                        {GHANA_REGIONS[getRegionKey(agent?.region)]?.map(
                          (d) => (
                    ...
                    >
                      ...
                      <SelectContent className="z-[2000] rounded-xl border-none shadow-2xl">
                        ...
                        {selectedDistrict !== "all" &&
                          GHANA_COMMUNITIES[selectedDistrict]?.map((c) => (
                    ...
                    <Select
                      ...
                      onValueChange={setSelectedFarmType}
                    >
                      <SelectTrigger
                        className={`w-full sm:w-40 h-11 ${inputBaseClasses} ${darkMode ? "bg-white/5 border-white/10" : "bg-gray-50"}`}
                      >
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      ...
                      <Button
                        ...
                        className={GROWER_DIRECTORY_ADD_BTN}
                      >
                ...
                {(statusFilter ||
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-2">
                    ...
                    {selectedDistrict !== "all" && (
                      <Badge
                        variant="outline"
                        className="border-gray-500/30 text-gray-500"
                      >
                        {selectedDistrict}
                      </Badge>
                    )}
            ...
            <div
              className={`mb-4 p-4 rounded-lg flex flex-wrap gap-6 items-center border ${darkMode ? "bg-[#002f37] border-[#7ede56]/20" : "bg-[#7ede56]/8 border-[#7ede56]/25"}`}
            >
              <p
                className={`text-sm font-semibold mr-2 ${darkMode ? "text-[#7ede56]" : "text-[#002f37]"}`}
              >
                Quick Actions Guide:
              </p>

              <div className="flex items-center gap-2">
                <div className={`p-1.5 rounded-md ${GROWER_DIRECTORY_LEGEND_ICON(darkMode)}`}>
            ...
            <Card
              className={`${sectionCardClass} border-0 shadow-xl overflow-hidden`}
            >
              <div className="overflow-hidden">
                <div className="p-6 border-b border-[#7ede56]/10 flex items-center justify-between bg-[#7ede56]/5">
                  <div>
                    <h3
                      className={`text-lg font-bold tracking-tight ${darkMode ? "text-white" : "text-gray-900"}`}
                    >
                      Grower Directory
                    </h3>
                    <p
                      className={`text-xs mt-1 font-medium ${darkMode ? "text-gray-400" : "text-gray-500"}`}
                    >
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant="outline"
                      className={`${darkMode ? "bg-[#7ede56]/10 border-[#7ede56]/25 text-[#7ede56]" : "bg-[#7ede56]/10 border-[#7ede56]/30 text-[#002f37]"} border px-3 py-1`}
                    >
                    ...
                    >
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        farmer.profile_picture ? (
                          ...
                          <div
                            className={`w-11 h-11 rounded-xl flex items-center justify-center text-xs font-bold text-white shrink-0 ${farmer.displayStatus === "Completed" ? "bg-emerald-500" : farmer.displayStatus === "Pending" ? "bg-amber-500" : "bg-indigo-500"}`}
                          >
                            {farmer.name
                              ?.split(" ")
                              .slice(0, 2)
                        ...
                        <div className="min-w-0 flex-1">
                          <p
                            className={`font-bold text-sm truncate ${darkMode ? "text-gray-100" : "text-gray-900"}`}
                          >
                            {farmer.name}
                          </p>
                          <p
                            className={`text-xs font-mono ${darkMode ? "text-emerald-400" : "text-emerald-600"}`}
                          >
                            {getDisplayId(farmer)}
                          </p>
                          <p
                        ...
                        <Badge
                          className={`shrink-0 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${farmer.displayStatus === "Completed" ? "bg-emerald-500/10 text-emerald-500" : farmer.displayStatus === "Pending" ? "bg-amber-500/10 text-amber-500" : "bg-indigo-500/10 text-indigo-500"}`}
                        >
                      ...
                      <div className="flex flex-wrap gap-2 pt-3 border-t border-gray-200 dark:border-white/10">
                        <Button
                          ...
                          className={`flex-1 min-w-[calc(33%-4px)] h-9 ${growerDirectoryMobileActionBtn(darkMode)}`}
                        >
                        ...
                        <Button
                          ...
                          className={`flex-1 min-w-[calc(50%-4px)] h-9 ${growerDirectoryMobileActionBtn(darkMode)}`}
                        >
                        ...
                        <Button
                          ...
                          disabled={pendingDeletionByFarmerId.has(
                            (farmer._id || farmer.id)?.toString(),
                          )}
                ...
                <div className="overflow-auto max-h-[65vh] relative hidden md:block">
                  <Table>
                    <TableHeader className={`sticky top-0 z-20 ${GROWER_DIRECTORY_HEADER}`}>
                      <TableRow className="border-none hover:bg-transparent">
                        <TableHead className={GROWER_DIRECTORY_HEAD_CELL}>#</TableHead>
                        <TableHead className={GROWER_DIRECTORY_HEAD_CELL}>
                          Farmer Details
                        </TableHead>
                        <TableHead className={GROWER_DIRECTORY_HEAD_CELL}>
                          Phone Number
                        ...
                        <TableHead className={`${GROWER_DIRECTORY_HEAD_CELL} text-right`}>
                          Actions
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                        ...
                        >
                            ...
                            <div className="flex items-center gap-4">
                              <div className="relative">
                                ...
                                farmer.profile_picture ? (
                                  ...
                                  <div
                                    className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-bold text-white shadow-lg transition-transform group-hover:scale-110 group-hover:rotate-3 ${farmer.displayStatus === "Completed" ? "bg-emerald-500" : farmer.displayStatus === "Pending" ? "bg-amber-500" : "bg-indigo-500"}`}
                                  >
                                    {farmer.name
                                      ?.split(" ")
                                      .slice(0, 2)
                              ...
                              <div>
                                <p
                                  className={`font-bold text-sm ${darkMode ? "text-gray-100" : "text-gray-900"}`}
                                >
                                  {farmer.name}
                                </p>
                                <p
                                  className={`text-xs font-mono mt-0.5 ${darkMode ? "text-emerald-400" : "text-emerald-600"}`}
                                >
                                  {getDisplayId(farmer)}
                                </p>
                              </div>
                          ...
                          <TableCell>
                            <div className="flex flex-col">
                              <Badge
                                variant="outline"
                                className={`w-fit text-[10px] uppercase font-bold tracking-wider mb-1 px-1.5 py-0 ${darkMode ? "border-indigo-500/30 text-indigo-400" : "border-indigo-200 text-indigo-700 bg-indigo-50"}`}
                              >
                                {farmer.farmType}
                              </Badge>
                              <span
                          ...
                          <TableCell>
                            <div className="flex flex-col gap-1.5 items-start">
                              <Badge
                                className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border-0 ring-1 ring-inset ${
                                  farmer.displayStatus === "Completed"
                                    ? "bg-emerald-500/10 text-emerald-500 ring-emerald-500/20"
                                    : farmer.displayStatus === "Pending"
                                      ? "bg-amber-500/10 text-amber-500 ring-amber-500/20"
                                      : farmer.displayStatus === "Matched"
                          ...
                          <TableCell className="text-right">
                            <div className="flex items-center justify-end gap-2 flex-wrap min-w-0 md:min-w-[320px]">
                              ...
                              <Button
                                ...
                                variant="ghost"
                                onClick={() =>
                                  handleRequestFarmerDeletion(farmer)
                                }
                                disabled={pendingDeletionByFarmerId.has(
                                  (farmer._id || farmer.id)?.toString(),
                                )}
                                ...
                                title={
                                  pendingDeletionByFarmerId.has(
                                    ...
                                    : "Request grower deletion (admin approval required)"
                                }
                              >
                  ...
                  {filteredFarmers.length === 0 && (
                    ...
                    >
                      <div
                        className={`w-20 h-20 mx-auto mb-6 rounded-[2rem] flex items-center justify-center rotate-12 transition-transform hover:rotate-0 duration-500 ${darkMode ? "bg-white/5 text-emerald-400" : "bg-emerald-50 text-emerald-500"}`}
                      >
          ...
          <TabsContent value="visits" className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
              <div>
                ...
                >
                  Chronological record of recent farm inspections
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  ...
                  {selectedVisits.size > 0
                    ? `Excel (${selectedVisits.size})`
                    : "Excel All"}
                </Button>
                <Button
                  onClick={() => handleLogVisit()}
                  className="bg-[#065f46] hover:bg-[#065f46]/90 text-white shadow-lg shadow-emerald-500/20 h-9 sm:h-10 px-3 sm:px-4 text-xs sm:text-sm border-none"
                >
              ...
              <div className="flex items-center gap-4 py-2 px-8">
                <div className="flex items-center gap-2">
                  <Checkbox
                    id="selectAllVisits"
                    checked={
                      selectedVisits.size === visitLogs.length &&
                      visitLogs.length > 0
                    }
                    onCheckedChange={(checked: boolean) => {
                      if (checked) {
                        setSelectedVisits(
                          new Set(visitLogs.map((v: any) => v._id || v.id)),
                        );
                      } else {
                  ...
                  />
                  <Label
                    htmlFor="selectAllVisits"
                    className={`text-xs font-bold cursor-pointer ${darkMode ? "text-gray-400" : "text-gray-500"}`}
                  >
                    Select All Records
                  </Label>
                </div>
                ...
                <>
                  {visitLogs.slice(0, 5).map((visit: any, index: number) => (
                    <div key={visit._id || index} className="group relative">
                      {/* Timeline Node */}
                      <div
                        className={`absolute -left-6 sm:-left-8 top-1.5 w-6 h-6 sm:w-7 sm:h-7 rounded-full border-4 flex items-center justify-center z-10 transition-transform group-hover:scale-110 ${
                          darkMode
                            ? "bg-[#002f37] border-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]"
                            : "bg-white border-emerald-500 shadow-md"
                        }`}
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${visit.status === "Completed" ? "bg-emerald-500" : "bg-amber-500"}`}
                        />
                      </div>
                            ...
                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
                              <div className="flex-1 space-y-2">
                                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                                  <span
                                    className={`text-[10px] font-bold opacity-40 font-mono ${darkMode ? "text-white" : "text-gray-900"}`}
                                  >
                                    #{visitLogs.length - index}
                                  </span>
                                  <Badge
                                    variant="outline"
                                    className={`text-[9px] sm:text-[10px] font-mono px-1.5 sm:px-2 py-0.5 ${darkMode ? "bg-white/5 border-white/10 text-emerald-400" : "bg-gray-50 text-emerald-600"}`}
                                  >
                                    {visit.farmer?.lyncId || visit.lyncId}
                                  </Badge>
                                  <h4
                                    className={`text-sm sm:text-base font-bold transition-colors ${darkMode ? "text-gray-100 group-hover:text-emerald-400" : "text-gray-900 group-hover:text-emerald-600"}`}
                                  >
                                    {visit.farmer?.name || visit.farmerName}
                                  </h4>
                                  <span
                                    className={`hidden sm:inline text-xs ${darkMode ? "text-gray-500" : "text-gray-400"}`}
                                  >
                                    •
                                  </span>
                                  <Badge
                                    className={`text-[9px] sm:text-[10px] uppercase tracking-widest ${darkMode ? "bg-emerald-500/10 text-emerald-400 border-0" : "bg-emerald-50 text-emerald-700"}`}
                                  >
                                    {visit.purpose}
                                  </Badge>
                                </div>
                                ...
                                {visit.challenges && (
                                  <div
                                    className={`mt-2 p-3 rounded-lg border text-sm ${darkMode ? "bg-amber-500/5 border-amber-500/10 text-amber-200/70" : "bg-amber-50 border-amber-100 text-amber-700"}`}
                                  >
                                ...
                                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 pt-2">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span
                                      className={`text-[11px] sm:text-xs font-medium ${darkMode ? "text-gray-400" : "text-gray-500"}`}
                                    >
                                      {new Date(visit.date).toLocaleDateString(
                                        undefined,
                                        {
                              ...
                              <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-start gap-4 md:gap-3 min-w-full md:min-w-[120px] pt-3 md:pt-0 border-t md:border-t-0 border-white/5">
                                <div
                                  className={`px-3 py-1 rounded-full text-[9px] sm:text-[10px] font-bold uppercase tracking-widest ring-1 ring-inset ${
                  ...
                  {visitLogs.length === 0 && (
                    ...
                    >
                      <div
                        className={`w-20 h-20 mx-auto mb-6 rounded-[2rem] flex items-center justify-center rotate-12 ${darkMode ? "bg-white/5 text-emerald-400" : "bg-emerald-50 text-emerald-500"}`}
                      >
                      ...
                      <p className="text-sm max-w-xs mx-auto opacity-70">
                        No field visits have been recorded yet. Start by logging
                        your first inspection.
                      </p>
                      <Button
                        onClick={() => handleLogVisit()}
                        className="mt-6 bg-[#065f46] border-none"
                      >
                        Log First Visit
                      </Button>
                    </div>
                ...
                <div className="agent-table-scroll overflow-x-auto relative custom-scrollbar">
                  <Table className="min-w-[800px] lg:min-w-full">
                    <TableHeader className="sticky top-0 z-20 bg-[#065f46] shadow-md">
                      <TableRow className="border-none hover:bg-transparent">
                        <TableHead className="text-white font-black text-[10px] uppercase tracking-widest py-4 px-6">
                          Farmer
                        </TableHead>
                        <TableHead className="text-white font-black text-[10px] uppercase tracking-widest py-4 px-6 text-center">
                          Type
                        </TableHead>
                        <TableHead className="text-white font-black text-[10px] uppercase tracking-widest py-4 px-6">
                          Date
                        ...
                        <TableHead className="text-right text-white font-black text-[10px] uppercase tracking-widest py-4 px-6">
                          Actions
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {reports.length === 0 ? (
                        <TableRow>
                          ...
                          >
                            No reports found. Upload a report from the Farmers
                            Directory.
                          </TableCell>
                        </TableRow>
                      ) : (
                        reports.map((report: any, index: number) => (
                          <TableRow
                            ...
                            className={`${darkMode ? "border-white/5 hover:bg-white/5" : "hover:bg-gray-50"} transition-colors`}
                          >
                            <TableCell className="font-medium">
                              <div className="flex flex-col">
                                ...
                                >
                                  {report.farmer?.name || "Unknown Farmer"}
                                </span>
                                <span
                                  className={`text-[10px] ${darkMode ? "text-gray-500" : "text-gray-400"}`}
                                >
                                  ID: {report.farmer?._id || "N/A"}
                                </span>
                              </div>
                            </TableCell>
                            ...
                            <TableCell
                              className={`max-w-[200px] truncate ${darkMode ? "text-gray-400" : "text-gray-500"} text-xs`}
                            >
                              {report.notes}
                            </TableCell>
                            <TableCell className="text-right">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    exportToPDF({
                                      ...report,
                                      farmerName:
                                        report.farmer?.name || "Farmer",
                                      agentName:
                                        agent?.name || "AgriLync Agent",
            ...
            >
              <div className="overflow-hidden">
                <div className="hidden md:flex p-6 border-b border-white/5 items-center justify-between bg-indigo-500/5">
                  <div>
                    ...
                    <p
                      className={`text-xs mt-1 font-medium ${darkMode ? "text-gray-400" : "text-gray-500"}`}
                    >
                      Monitoring {filteredFarms.length} ongoing agricultural
                      operations
                    </p>
                  </div>
                  <Badge
                    variant="outline"
                    className={`${darkMode ? "bg-indigo-500/10 border-indigo-500/20 text-indigo-400" : "bg-indigo-50 border-indigo-100 text-indigo-700"} border px-3 py-1`}
                  >
                ...
                <div className="agent-table-scroll overflow-auto max-h-[65vh]">
                  <Table
                    className={`border ${darkMode ? "border-white/10" : "border-gray-200"}`}
                  >
                    <TableHeader className="sticky top-0 z-20 bg-[#065f46] shadow-md">
                      <TableRow className="border-none hover:bg-transparent">
                        <TableHead className="text-white font-black text-[10px] uppercase tracking-widest py-4 px-6 border-r border-white/10">
                          Farm Information
                        </TableHead>
                        <TableHead className="text-white font-black text-[10px] uppercase tracking-widest py-4 px-6 text-center border-r border-white/10">
                          Health Status
                        </TableHead>
                        <TableHead className="text-white font-black text-[10px] uppercase tracking-widest py-4 px-6 border-r border-white/10">
                          Next Visit
                    ...
                    <TableBody>
                      {filteredFarms.length === 0 ? (
                          ...
                          <TableCell colSpan={4} className="h-64 text-center">
                            <div className="flex flex-col items-center justify-center space-y-3">
                              ...
                              <p className="text-sm font-black text-gray-400 uppercase tracking-widest">
                                No active farms discovered
                              </p>
                              <p className="text-[10px] text-gray-400">
                                Registered farms with ongoing missions will
                                appear here.
                              </p>
                            </div>
                          </TableCell>
                        ...
                        filteredFarms.map((farm: any) => (
                          <TableRow
                            key={farm._id}
                            className={`${darkMode ? "border-white/5 hover:bg-emerald-500/5" : "hover:bg-gray-50"} transition-all duration-300 group ring-inset`}
                          >
                            <TableCell
                              className={`py-4 px-6 border-r ${darkMode ? "border-white/10" : "border-gray-100"}`}
                            >
                              <div className="flex items-center gap-4">
                                <div className="h-14 w-14 rounded-2xl bg-gray-100 dark:bg-gray-800 overflow-hidden shadow-lg border-2 border-white dark:border-white/10 group-hover:scale-110 transition-transform">
                                  <img
                                    src={
                                      ...
                                      `https://api.dicebear.com/7.x/initials/svg?seed=${farm.name}`
                                    }
                                    alt={farm.name}
                                    className="w-full h-full object-cover"
                                  />
                                ...
                                <div>
                                  <p
                                    className={`text-sm font-black tracking-tight ${darkMode ? "text-gray-100" : "text-[#002f37]"}`}
                                  >
                                    {farm.name}
                                  </p>
                                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">
                                    {farm.farmer?.name || "Assigned Grower"}
                                  </p>
                                </div>
                              </div>
                            </TableCell>
                            <TableCell
                              className={`py-4 border-r ${darkMode ? "border-white/10" : "border-gray-100"}`}
                            >
                              <div className="flex flex-col items-center gap-1.5 px-4 w-32 mx-auto">
                                <div className="flex items-baseline gap-1">
                                  <span
                                    className={`text-xs font-black ${darkMode ? "text-[#7ede56]" : "text-[#065f46]"}`}
                                  >
                                    {farm.health || 0}%
                                  </span>
                                ...
                                <div className="h-1 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                                  <div
                                    className={`h-full ${darkMode ? "bg-[#7ede56]" : "bg-[#065f46]"}`}
                                    style={{ width: `${farm.health || 0}%` }}
                                  ></div>
                                </div>
                              </div>
                            </TableCell>
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <div className="flex flex-col">
                                  <p
                                    className={`text-xs font-black ${darkMode ? "text-gray-200" : "text-[#002f37]"}`}
                                  >
                                    {farm.nextVisit || "Awaiting Schedule"}
                                  </p>
                ...
                <div className="p-6 border-b border-white/5 flex items-center justify-between bg-emerald-500/5">
                  <div>
                    ...
                    <p
                      className={`text-xs mt-1 font-medium ${darkMode ? "text-gray-400" : "text-gray-500"}`}
                    >
                      Managing {filteredMatches.length} investor-grower
                      relationships
                    </p>
                  </div>
                  <Badge
                    variant="outline"
                    className={`${darkMode ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-emerald-50 border-emerald-100 text-emerald-700"} border px-3 py-1`}
                  >
                    <div className="w-2 h-2 rounded-full bg-[#7ede56] mr-2 animate-pulse" />
                  ...
                  >
                    <TableHeader className="bg-[#065f46] shadow-md">
                      <TableRow className="border-none hover:bg-transparent">
                        ...
                        <TableHead className="text-white font-black text-[10px] uppercase tracking-widest py-4 text-center border-r border-white/10">
                          Value
                        </TableHead>
                        <TableHead className="text-white font-black text-[10px] uppercase tracking-widest py-4 px-6 border-r border-white/10">
                          Status
                      ...
                      {filteredMatches.length === 0 ? (
                        ...
                        filteredMatches.map((match: any) => (
                          <TableRow
                            ...
                            className={`${darkMode ? "border-white/5 hover:bg-emerald-500/5" : "hover:bg-gray-50"} border-b`}
                          >
                            <TableCell
                              className={`py-4 px-6 font-medium text-[11px] border-r ${darkMode ? "border-white/10" : "border-gray-100"}`}
                            >
                              {match.matchDate}
                            </TableCell>
                            <TableCell
                            ...
                            <TableCell
                              className={`py-4 text-center border-r ${darkMode ? "border-white/10" : "border-gray-100"}`}
                            >
      ...
      <AddFarmerModal
        open={isAddFarmerModalOpen}
        onOpenChange={setIsAddFarmerModalOpen}
        onSuccess={fetchData}
      />
      <ViewFarmerModal
        open={viewModalOpen}
      ...
      <UploadReportModal
        ...
        onUpload={() => refetchReports()}
      />
      <MediaUploadModal
        open={uploadMediaModalOpen}
        onOpenChange={setUploadMediaModalOpen}
        farmer={selectedFarmer}
        onSuccess={fetchData}
      />
      <FarmJourneyModal
      ...
      <Dialog open={fieldVisitModalOpen} onOpenChange={setFieldVisitModalOpen}>
        <DialogContent
          className={`agent-modal-mobile w-full max-w-[100vw] overflow-x-hidden md:max-w-5xl p-0 overflow-hidden border-0 flex flex-col max-md:h-full max-md:max-h-[100dvh] max-md:rounded-none ${darkMode ? "bg-[#002f37]" : "bg-white"}`}
        >
          ...
          <div className="flex-1 p-4 sm:p-8 space-y-4 sm:space-y-6 overflow-y-auto overflow-x-hidden min-h-0">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent
                    className={darkMode ? "bg-gray-900 border-white/10" : ""}
                  >
                    {farmers.map((farmer: any, index: number) => (
                      <SelectItem
                        key={
                          getFarmerMongoId(farmer) || `select-farmer-${index}`
                        }
                        value={getFarmerMongoId(farmer)}
              ...
              <div className="space-y-2">
                <Label
                  className={`text-xs font-bold uppercase tracking-wider ${darkMode ? "text-gray-400" : "text-gray-500"}`}
                >
                  Lync ID Identification
                </Label>
                <div
                  className={`h-11 flex items-center px-3 rounded-lg border font-mono text-sm ${
                    visitForm.lyncId
                      ? darkMode
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <Calendar
                    className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 transition-colors ${darkMode ? "text-gray-500 group-focus-within:text-emerald-500" : "text-gray-400 group-focus-within:text-emerald-500"}`}
                  />
                  <Input
                    type="date"
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setVisitForm({ ...visitForm, hoursSpent: e.target.value })
                    }
                    placeholder="e.g., 1.5"
                    className={`pl-10 h-11 ${inputBaseClasses}`}
                  />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setVisitForm({ ...visitForm, purpose: val })
                  }
                >
                  <SelectTrigger className={`h-11 ${inputBaseClasses}`}>
                  ...
                  <SelectContent
                    className={
                      darkMode ? "bg-[#002f37] border-white/10 text-white" : ""
                    }
                  >
                    ...
                    <SelectItem value="Irrigation check">
                      Irrigation Check
                    </SelectItem>
                    <SelectItem value="Harvest assessment">
                      Harvest Assessment
                    </SelectItem>
              ...
              {visitForm.purpose === "Other" ? (
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(val) =>
                      setVisitForm({ ...visitForm, status: val })
                    }
                  >
                    <SelectTrigger className={`h-11 ${inputBaseClasses}`}>
                    ...
                    <SelectContent
                      className={
                        darkMode
                          ? "bg-[#002f37] border-white/10 text-white"
                          : ""
                      }
                    >
                      <SelectItem value="Completed">Completed</SelectItem>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setVisitForm({ ...visitForm, challenges: e.target.value })
                }
            ...
            <div className="space-y-2">
                ...
                {visitImages.map((img, idx) => (
                  <div key={idx} className="relative group">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setVisitImages(visitImages.filter((_, i) => i !== idx))
                      }
              ...
              {visitImages.length > 0 && (
                <p className="text-xs text-gray-500">
                  {visitImages.length}/6 photos uploaded
                </p>
              )}
            </div>
      ...
      <Dialog
        open={visitDetailModalOpen}
        onOpenChange={setVisitDetailModalOpen}
      >
        <DialogContent
          className={`agent-modal-mobile w-full max-w-[100vw] overflow-x-hidden md:max-w-5xl max-h-[90vh] overflow-y-auto max-md:rounded-none ${darkMode ? "bg-[#002f37] border-white/10 text-white" : "bg-white"}`}
        >
          <DialogHeader className="pr-10 sm:pr-12">
            <DialogTitle className="flex flex-wrap items-center gap-2 sm:gap-3">
              <Badge
                variant="outline"
                className={`text-xs font-mono shrink-0 ${darkMode ? "bg-white/5 border-white/10 text-emerald-400" : "bg-gray-50 text-emerald-600"}`}
              >
                {selectedVisit?.farmer?.lyncId || selectedVisit?.lyncId}
              </Badge>
              <span className="text-lg sm:text-xl font-bold break-words">
                {selectedVisit?.farmer?.name || selectedVisit?.farmerName}
              </span>
            </DialogTitle>
            <DialogDescription
            ...
            <div className="space-y-6 py-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 pb-4 border-b border-white/5">
                ...
                <div className="space-y-1">
                  ...
                  <Badge
                    className={`text-[10px] uppercase tracking-widest ${selectedVisit.status === "Completed" ? "bg-emerald-500/10 text-emerald-500 border-0" : "bg-amber-500/10 text-amber-500 border-0"}`}
                  >
              ...
              <div className="space-y-2">
                <Label className="text-[11px] font-bold uppercase tracking-wider text-emerald-500">
                  Inspection Purpose
                </Label>
              ...
              {selectedVisit.challenges && (
                <div
                  className={`p-4 rounded-xl border ${darkMode ? "bg-amber-500/5 border-amber-500/10" : "bg-amber-50 border-amber-100"}`}
                >
                  ...
                  <p
                    className={`text-sm ${darkMode ? "text-amber-200/70" : "text-amber-800"}`}
                  >
                    {selectedVisit.challenges}
                  </p>
                </div>
              ...
              {selectedVisit.visitImages &&
                selectedVisit.visitImages.length > 0 && (
                  <div className="space-y-3">
                    <Label className="text-[11px] font-bold uppercase tracking-wider text-emerald-500">
                      Visit Gallery
                    </Label>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {selectedVisit.visitImages.map(
                        (img: string, idx: number) => (
                          <div
                            key={idx}
                            className="aspect-square rounded-lg overflow-hidden border border-white/10 group relative cursor-pointer"
