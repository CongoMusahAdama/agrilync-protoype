...
const DisputeManagement: React.FC = () => {
  ...
  const summaryCards = [
    { title: 'Total Disputes', value: totalDisputes, icon: AlertTriangle, status: 'all', bg: 'bg-[#7ede56]' }, // Greenish
    { title: 'Pending', value: pendingDisputes, icon: Clock, status: 'Pending', bg: 'bg-[#f97316]' }, // Orange
    { title: 'Under Review', value: underReviewDisputes, icon: Search, status: 'Under Review', bg: 'bg-[#0ea5e9]' }, // Blue
    { title: 'Resolved', value: resolvedDisputes, icon: CheckCircle, status: 'Resolved', bg: 'bg-[#10b981]' }, // Emerald
    { title: 'Escalated', value: escalatedDisputes, icon: TrendingUp, status: 'Escalated', bg: 'bg-[#ef4444]' }, // Red
  ];
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
        {summaryCards.map((card, idx) => (
          <Card
            key={idx}
            className={`cursor-pointer transition-all hover:shadow-lg hover:scale-[1.02] border-none text-white ${card.bg} ${statusFilter === card.status ? 'ring-4 ring-white/30' : ''}`}
