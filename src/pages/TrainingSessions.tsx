...
const TrainingSessions = () => {
  ...
  const [activeTab, setActiveTab] = useState<'consultation' | 'learning' | 'advisory'>('consultation');
  // Mock user location - In a real app, this would come from user profile or geolocation
  const [userLocation, setUserLocation] = useState('Ashanti');
  ...
  const advisors = [
    {
      ...
      expertise: ['Maize', 'Cocoa', 'Organic Fertilizer'],
      mode: 'In-person / Virtual'
    },
    {
      id: 2,
      name: 'Sarah Amankwah',
      specialization: 'Agribusiness & Market Linkages',
      region: 'Bono East',
      availability: 'Tue - Sat, 9AM - 5PM',
      expertise: ['Poultry', 'Vegetables', 'Financial Planning'],
      mode: 'Virtual Only'
    },
    {
      id: 3,
      ...
      expertise: ['Greenhouse', 'Hydroponics', 'Drip Irrigation'],
      mode: 'In-person / Virtual'
    },
    {
      id: 4,
      name: 'Kwesi Appiah',
      specialization: 'Farm Machinery & mechanization',
      region: 'Ashanti',
      availability: 'Mon - Sat, 7AM - 5PM',
      expertise: ['Tractors', 'Harvesters', 'Irrigation Pumps'],
      mode: 'In-person'
    }
  ];

  const filteredAdvisors = advisors.filter(advisor => advisor.region === userLocation);
    ...
    <DashboardLayout activeSidebarItem="training-sessions" title="Training & Advisory">
      ...
      <div className="w-full p-3 sm:p-4 md:p-6 max-w-7xl mx-auto">
        <div className="mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className={`p-1 grid grid-cols-3 sm:flex w-full sm:w-auto bg-opacity-50 backdrop-blur-sm rounded-xl border ${darkMode ? 'bg-[#01343c] border-gray-700' : 'bg-white border-gray-200 shadow-sm'}`}>
            {[
              ...
              <button
                ...
                className={`flex items-center justify-center sm:justify-start gap-2 px-2 sm:px-6 py-2.5 rounded-lg text-[10px] sm:text-sm font-bold transition-all duration-300 ${activeTab === tab.id
          ...
          <div className="flex items-center gap-2 mt-4 sm:mt-0">
            <Badge variant="outline" className={`px-4 py-1.5 rounded-full ${darkMode ? 'border-[#7ede56] text-[#7ede56]' : 'border-[#002f37] text-[#002f37]'}`}>
              {activeTab === 'consultation' ? `Expert Agents Available` :
                activeTab === 'learning' ? `${filteredSessions.length} Programs Available` : 'AI Advisor Online'}
            </Badge>
          </div>
        </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Showing agents available in <span className="font-bold text-[#7ede56]">{userLocation} Region</span>
              </p>
            </div>
            {filteredAdvisors.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredAdvisors.map((advisor) => (
                  <Card key={advisor.id} className={`${darkMode ? 'bg-[#002f37] border-gray-700' : 'bg-white shadow-md'} hover:-translate-y-1 transition-all duration-300`}>
                    <CardHeader>
                      <div className="flex items-center gap-4">
                        <Avatar className="h-12 w-12 border-2 border-[#7ede56]">
                          <AvatarFallback className="bg-[#002f37] text-white">
                            {advisor.name.split(' ').map(n => n[0]).join('')}
                          </AvatarFallback>
                        </Avatar>
                        <div>
                          <CardTitle className={`text-lg ${darkMode ? 'text-white' : 'text-gray-900'}`}>{advisor.name}</CardTitle>
                          <CardDescription className={darkMode ? 'text-[#7ede56]' : 'text-[#002f37]'}>{advisor.specialization}</CardDescription>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="flex flex-col gap-2 text-sm">
                        <div className={`flex items-center gap-2 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                          ...
                          <span>{advisor.region} Region</span>
                        </div>
                        <div className={`flex items-center gap-2 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                      ...
                      <div className="pt-4 flex gap-2">
                        <Button
                          className="flex-1 bg-[#7ede56] hover:bg-[#6bc947] text-[#002f37] font-bold"
                          onClick={() => {
                        ...
                        <Button
                          variant="outline"
                          className="flex-1 border-[#7ede56] text-[#7ede56] hover:bg-[#7ede56]/10"
                          onClick={() => {
                            setSelectedAdvisor(advisor);
                            setBookingSuccess(true);
                          }}
              ...
              <Card className={`${darkMode ? 'bg-[#002f37] border-gray-700' : 'bg-white shadow-md'}`}>
                <CardContent className="p-8 text-center flex flex-col items-center">
                  <MapPin className={`h-12 w-12 mb-4 ${darkMode ? 'text-gray-600' : 'text-gray-300'}`} />
                  <h3 className={`text-lg font-bold mb-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>No Agents Found in {userLocation}</h3>
                  <p className={`max-w-md mx-auto ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    We couldn't find any agents specifically assigned to your region at the moment. Try checking back later or browse all available agents.
                  </p>
                  <Button
                    variant="outline"
                    className={`mt-4 ${darkMode ? 'border-[#7ede56] text-[#7ede56]' : 'border-[#002f37] text-[#002f37]'}`}
                    onClick={() => setUserLocation(userLocation === 'Ashanti' ? 'Greater Accra' : 'Ashanti')} // Simple toggle for demo
                  >
                    View All Regions
                  </Button>
                </CardContent>
          ...
          activeTab === 'learning' && (
            <div className="space-y-8">
              <section className="space-y-6">
                <div className="flex items-center justify-between">
                  <h2 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Available Programs</h2>
                </div>
              ...
              <section className="space-y-6 pt-8 border-t border-gray-100 dark:border-gray-800">
                <div className="flex items-center justify-between">
                  <h2 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>My Training Schedule</h2>
                </div>
                <Card className={darkMode ? 'bg-[#002f37] border border-gray-700' : 'bg-white shadow-sm'}>
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                                  ...
                                  {training.status === 'Completed' ? (
                                    ...
                                    <Button variant="outline" size="sm" className={`text-xs ${darkMode ? 'border-gray-600 text-white hover:bg-gray-800' : 'text-gray-700 hover:bg-gray-100'}`}>
                                      Reschedule
                                    </Button>
                                  )}
                                </div>
...

        {
          activeTab === 'advisory' && (
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
              ...
              {/* Main Center: AI Chat Assistant - Fixed Viewport Design */}
              <div className={`xl:col-span-2 flex flex-col ${isMobile ? 'h-[500px]' : 'h-[calc(100vh-220px)] min-h-[600px]'} sticky top-24`}>
                <Card className={`flex-1 flex flex-col border-none shadow-2xl overflow-hidden ${darkMode ? 'bg-[#01343c]/30' : 'bg-gray-100/30'}`}>
                  {/* Chat Header */}
